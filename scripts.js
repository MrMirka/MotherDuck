...
function checkTurn(){
	if(positions.length > 8){
		...
		let preLast = positions[positions.length-8];

		let moveY = (panYTouch[panYTouch.length-1] - panYTouch[panYTouch.length-8]) / 2; 
		let moveX = positions[positions.length-1] - positions[positions.length-8];
